...
$this->title = ($model->isNewRecord) ? 'Add New Bank' : 'Update Bank ' . $model->name . ' details.';
$this->params['breadcrumbs'][] = ['label' => 'Bank', 'url' => ['bank']];
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="card bd-0 shadow-base widget-14 ht-100p">
    <div class="card-body">
        <?php $form = ActiveForm::begin(['id' => 'form-bank', 'options' => ['enctype' => 'mutipart/form-data', 'class' => 'form-horizontal form-bordered']]); ?>
        ...
        <?= $form->field($model, 'branch', ['options' => ['class' => 'form-group']])->begin() ?>
        <?= Html::activeLabel($model, 'branch', ['class' => 'col-lg-3 col-sm-3 col-xs-3 control-label']); ?>
        <div class="col-lg-6 col-sm-6 col-xs-6">
            <?= Html::activeTextInput($model, 'branch', ['class' => 'form-control']) ?>
            <?= Html::error($model, 'branch', ['class' => 'error help-block']) ?>
        </div>
        <?= $form->field($model, 'branch')->end() ?>
